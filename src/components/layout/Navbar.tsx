      ...
      <div className="container mx-auto px-4 lg:px-8">
        <nav className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map(link => <Link key={link.name} to={link.path} className={`relative text-sm font-medium transition-colors duration-300 ${location.pathname === link.path ? 'text-accent' : 'text-foreground/70 hover:text-foreground'}`}>
                {link.name}
                {location.pathname === link.path && <motion.div layoutId="activeNav" className="absolute -bottom-1 left-0 right-0 h-0.5 bg-accent-gradient" />}
              </Link>)}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            scale: 0.95
          }} onClick={() => setSearchOpen(true)} className="hidden md:flex items-center justify-center w-10 h-10 rounded-full bg-muted/50 hover:bg-muted transition-colors">
            ...
            <Link to="/wishlist">
              ...
              scale: 0.95
                ...
                {wishlistItems.length > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 flex items-center justify-center text-xs font-bold bg-accent-gradient text-accent-foreground rounded-full">
            ...
            scale: 0.95
          }} onClick={openCart} className="flex items-center justify-center w-10 h-10 rounded-full bg-muted/50 hover:bg-muted transition-colors relative">
            ...
            scale: 0.95
          }} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden flex items-center justify-center w-10 h-10 rounded-full bg-muted/50 hover:bg-muted transition-colors">
      ...
      <AnimatePresence>
        ...
        height: 0
            ...
            delay: index * 0.1
                  ...
                  <Link to={link.path} className={`text-lg font-medium transition-colors ${location.pathname === link.path ? 'text-accent' : 'text-foreground/70'}`}>
                    {link.name}
                  </Link>
                </motion.div>)}
            ...
            delay: 0.4
          }} onClick={handleLogout} className="text-lg font-medium text-destructive text-left">
                  Sign Out
